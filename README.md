...
### 기존 페이지 개선점
- 기존 페이지는 서로 다른 이벤트영역 (반짝할인,깜짝세일 등)으로 쭉쭉 내리는 방식으로 구현되어 있었습니다. 이전 영역을 보기 위해서는 다시 스크롤해 이동해야되는 불편함이 있었습니다.
- 스크롤탭을 도입했습니다. 메인페이지에서 특정 영역으로 스크롤하면 다음 탭으로 이동하도록 구현하는 방식으로 기존 페이지 UI를 개선해보았습니다.
 
 
### 이 프로젝트를 통해 배운점
- 처음으로 graphQL과 styled-components를 사용해보았습니다.
- 리액트를 제대로 공부하지 않은 상태에서 도입하여 기본적인 hooks만 사용했습니다. 코드 리뷰시에 customHooks등을 사용하라는 피드백을 받아 개인 toy프로젝트에 공부하여 적용했습니다.
- 메인페이지에서 무한 스크롤을 구현할 때 현재 데이터를 불러오고 있는 상태인지를 나타내는 fetching이라는 state를 사용하여 또 다른 data fetching 요청을 막았습니다. 추후 이것이
  쓰로틀링이라는 것을 알게되었습니다.
